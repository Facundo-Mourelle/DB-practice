...
as (
    ...
    group by customers.customerNumber, customers.city, 
...
SELECT sub.customerName AS custName, 
       sub.city, 
       sub.totalSpent 
FROM (
...
READS SQL DATA
begin
-- @res es una variable de sesion. Se puede usar un declare, pero esto es mas sencillo
	set @res := (
...
-- sin usar una variable de sesion:


drop function employee_of_the_month;

delimiter //
create function employee_of_the_month(yr year, mnth int) 
returns varchar(100) 
reads sql data 
begin 
...
select employee_of_the_month(2003, 3) as winner;
...
/*
Encontrar, para cada cliente de aquellas ciudades que comienzan por "N", la menor y
